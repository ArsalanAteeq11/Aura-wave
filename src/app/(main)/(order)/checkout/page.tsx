...
import { AlertCircle, CreditCard, Home, Landmark, Loader2, MapPin, Truck } from 'lucide-react';
...
const checkoutFormSchema = z.object({
    ...
    number: z.string().regex(/^(\+92|92|0)?\d{10}$/, "Enter a valid phone number"),
    ...
    paymentMethod: z.enum(["Cash On Delivery (COD)", "Bank Transfer", "JazzCash / EasyPaisa"], {
...
const Checkout = () => {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        try {
            const payload = {
                ...
                subtotalAmount: orderTotals.totalAmount + orderTotals.discountAmount - orderTotals.shippingCost,
            ...
            sessionStorage.setItem("orderDetails", JSON.stringify(payloadWithOrderId)) // Store order details in sessionStorage
            router.push("/success")
    ...
    const handleTotalUpdate = (totalAmount: number, discountCode: string, discountAmount: number, shippingCost: number) => {
        setOrderTotals({
            totalAmount,
            discountCode,
            discountAmount,
                    ...
                    <AccordionItem value="summary">
                        <AccordionTrigger className="text-base font-medium justify-between">
                            <div className="space-x-3">
                                ...
                                <span className="font-bold">{formatCurrency(orderTotals.totalAmount)}</span>
                            </div>
                        </AccordionTrigger>
                    ...
                    >
                        ...
                        <div className="space-y-2">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => setValue("province", value as typeof provinces[number], { shouldDirty: true })}
                            >
                                <SelectTrigger
                                    className={`w-full ${watch("province")
                            ...
                            <div className="flex items-center space-x-2 mt-2">
                                ...
                                />
                                <Label htmlFor="saveInfo" className="ml-2 text-base font-medium text-muted-foreground select-none cursor-pointer">
                        ...
                        <div className="space-y-2">
                            ...
                            {shippingOptions.map((option) => (
                                <div
                                    key={option.value}
                                    onClick={() => setValue("shippingMethod", option.value as "Free Delivery" | "Fast Delivery")}
                                    className={`flex items-center justify-between border rounded-lg p-4 transition-all cursor-pointer ${selectedShippingMethod === option.value
                        ...
                        <div className="space-y-3">
                            ...
                            {/* Cash on Delivery */}
                            <div
                                className={`border rounded-lg transition-all ${selectedPaymentMethod === 'Cash On Delivery (COD)'
                                    ? 'border-primary bg-primary/5'
                                    : 'border-border hover:border-primary/50'
                                    }`}
                            >
                                        ...
                                        >
                                            <div className="px-4 pb-4 pt-0 text-sm text-muted-foreground bg-muted/50">
                                                <div className="flex items-start">
                                                    ...
                                                    <p>
                                                        If the order is refused at the time of delivery, your order will not be
                                                        entertained next time or you must pay the delivery amount first.
                                                    </p>
                                                </div>
                                            </div>
                                        </motion.div>
                            ...
                            {/* Bank Transfer */}
                            <div
                                className={`border rounded-lg transition-all ${selectedPaymentMethod === 'Bank Transfer'
                                    ? 'border-primary bg-primary/5'
                                    : 'border-border hover:border-primary/50'
                                    }`}
                            >
                                    ...
                                    {selectedPaymentMethod === 'Bank Transfer' && (
                                        ...
                                        >
                                            <div className="px-4 pb-4 pt-0 text-sm bg-muted/50">
                                                <p className="mb-2 text-muted-foreground">Please transfer the total amount to:</p>
                                                <p className="mb-1">
                                                    <span className="font-medium">Bank Name:</span> Meezan Bank Limited
                                                </p>
                                                <p className="mb-1">
                                                    <span className="font-medium">Account Title:</span> Shaheryar Saud
                                                </p>
                                                <p className="mb-1">
                                                    <span className="font-medium">Account Number:</span> 99120112395956
                                                </p>
                                                <p className="mt-2">
                                                    Please make sure your payment directly into our bank account (Shaheryar Saud), use Order ID as the payment reference. Kindly share screenshot as payment proof on our WhatsApp No. ({siteConfig.socials.whatsapp}) or Email {siteConfig.email}
                                                </p>
                                            </div>
                                        </motion.div>
                                    )}
                            ...
                            <div
                                className={`border rounded-lg transition-all ${selectedPaymentMethod === 'JazzCash / EasyPaisa'
                                    ? 'border-primary bg-primary/5'
                                    : 'border-border hover:border-primary/50'
                                    }`}
                            >
                                    ...
                                    {selectedPaymentMethod === 'JazzCash / EasyPaisa' && (
                                        ...
                                        >
                                            <div className="px-4 pb-4 pt-0 text-sm bg-muted/50">
                                                ...
                                                <p className="mb-2 text-muted-foreground">Pay with JazzCash or Easypaisa</p>
                                                <p className="mb-1">
                                                    <span className="font-medium">Account Title:</span> Shaheryar Saud
                                                </p>
                                                <p className="mb-1">
                                                    <span className="font-medium">Account No:</span> 03182598013
                                                </p>
                                                <p className="mt-2 text-muted-foreground">
                                                    Kindly share the screenshot of the payment for further processing. Thank you!
                                                </p>
                                            </div>
                                        </motion.div>
                                    )}
                            ...
                            <div className="space-y-3">
                                {[
                                    ...
                                    <div
                                        ...
                                        className={`border rounded-lg transition-all cursor-pointer ${showBilling === (option.value === "different")
                                            ? "border-primary bg-primary/5"
                                            : "border-border hover:border-primary/50"
                                            }`}
                                    >
                                        <Label className="flex items-center space-x-3 p-4 cursor-pointer">
                                            <Input
                                                ...
                                                onChange={() => setShowBilling(option.value === "different")}
                                                className="h-4 w-4 text-primary"
                                            />
                                            <span className="flex items-center font-medium">
                                                {option.icon}
                            ...
                            <AnimatePresence>
                                {showBilling && (
                                    ...
                                    >
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div>
                                                <Input placeholder="First name" {...register("billing.firstName")} />
                                                {errors?.billing?.firstName && (
                                                    <p className="text-sm text-red-500 mt-1">{errors.billing.firstName.message}</p>
                                                )}
                                            </div>
                                            <div>
                                                <Input placeholder="Last name (optional)" {...register("billing.lastName")} />
                                            </div>
                                        </div>

                                        <div>
                                            <Input placeholder="Address" {...register("billing.address")} />
                                            {errors?.billing?.address && (
                                                <p className="text-sm text-red-500 mt-1">{errors.billing.address.message}</p>
                                            )}
                                        </div>

                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <div>
                                                <Input placeholder="City" {...register("billing.city")} />
                                                {errors?.billing?.city && (
                                                    <p className="text-sm text-red-500 mt-1">{errors.billing.city.message}</p>
                                                )}
                                            </div>
                                            <div>
                                                <Input placeholder="Postal code (optional)" {...register("billing.postalCode")} />
                                            </div>
                                        </div>

                                        <Input placeholder="Phone (optional)" {...register("billing.phone")} />
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                        ...
                        {/* Submit Button */}
                        <Button type="submit" className="w-full rounded text-base" disabled={isOrderLoading}>
                            {isOrderLoading ? "Submitting Order..." : `Complete order – ${formatCurrency(orderTotals.totalAmount)}`}
                        </Button>
                    </form>
                </div>
...
type Props = {
    ...
    onTotalUpdate?: (total: number, discountCode: string, discountAmount: number, shippingCost: number) => void
}

function CartSummaryContent({ cartItems, selectedShippingMethod = "Free Delivery", onTotalUpdate }: Props) {
    ...
    const subtotal = cartItems.reduce(
        (acc, item) => acc + (Number(item.discountedPrice ?? item.originalPrice) * item.quantity),
        0
    );

    const shippingOption = shippingOptions.find(opt => opt.value === selectedShippingMethod);
    ...
    }, [totalAmount, discountCode, discountAmount, shippingCost, onTotalUpdate, prevTotals]);

    return (
        <div className="space-y-4 p-2">
            {/* Cart Items */}
            {cartItems.map((item: CartItem) => (
                <div className="flex items-start gap-4" key={`${item._id}-${item.variantKey}`}>
                    <Image
                        src={item.imageUrl}
                        width={64}
                        height={64}
                    ...
                    <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium break-words leading-snug whitespace-normal text-wrap">{item.title}</p>
                        {item.variant && <p className="text-sm text-muted-foreground">{item.variant}</p>}
                        <p className="text-xs text-muted-foreground">Qty: {item.quantity}</p>
                    </div>
                    <p className="text-sm font-medium">{formatCurrency(Number(item.discountedPrice ?? item.originalPrice) * item.quantity)}</p>
                </div>
            ))}
